...
describe("GET /api/students/available-for-adoption", () => {
  test("should return students without a current sponsor", async () => {
    const s1 = await createTestStudent({ first_name: "Sponsored", status: "eligible" }, token);
    const s2 = await createTestStudent({ first_name: "Available", status: "eligible" }, token);
    // Sponsor s1
    ...
    const res = await request(app).get("/api/students/available-for-adoption").set("Authorization", `Bearer ${token}`);
  ...
  test("should return empty list when all are sponsored", async () => {
    const s1 = await createTestStudent({ status: "eligible" }, token);
    await request(app).post("/api/donor-mappings").send({
      student_id: s1.id,
