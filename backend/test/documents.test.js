...
// Create a small temp file for upload testing (use .pdf extension to pass MIME filter)
...
describe("POST /api/documents", () => {
  test("should upload a document and store metadata", async () => {
    ...
    expect(res.body.data.file_name).toBe("test-upload.pdf");
...
describe("GET /api/documents", () => {
  ...
  test("should filter by student_id", async () => {
    ...
    await request(app).post("/api/documents").set("Authorization", `Bearer ${token}`).field("student_id", s1.id.toString()).attach("file", tmpFile);
    await request(app).post("/api/documents").set("Authorization", `Bearer ${token}`).field("student_id", s2.id.toString()).attach("file", tmpFile);
    const res = await request(app).get(`/api/documents?student_id=${s1.id}`).set("Authorization", `Bearer ${token}`);
    expect(res.body.data.length).toBe(1);
  });
});
...
describe("DELETE /api/documents/:id", () => {
  test("should delete a document", async () => {
    ...
    expect(res.body.error).toBeNull();
    const check = await request(app).get(`/api/documents?student_id=${student.id}`).set("Authorization", `Bearer ${token}`);
